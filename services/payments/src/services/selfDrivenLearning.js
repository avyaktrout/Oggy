...
class SelfDrivenLearning {
    ...
    async _generateAssessment() {
        ...
        if (useTessaGeneration) {
            ...
            const difficultyTier = await adaptiveDifficultyScaler.getInstance(this.userId).selectDifficultyTier(
                this.recentAccuracy,
                this.userId
            );
            ...
            logger.debug('Requesting novel scenario from Tessa', {
                ...
                baseline_scale: adaptiveDifficultyScaler.getInstance(this.userId).baselineDifficultyScale,
    ...
    async _ensureBaselineLoaded() {
        ...
        if (this._baselineLoadedFor !== this.userId) {
            ...
            this._baselineLoadPromise = adaptiveDifficultyScaler.getInstance(this.userId).loadBaselineScale(this.userId);
        }
     ...
     * Scoped to this.userId for tenant isolation
     */
        ...
        if (targetCategory) {
            ...
            if (subtopic) {
                query_text = `SELECT content_structured
                   ...
                   ORDER BY RANDOM()
                   LIMIT 1`;
                query_params = [subtopic, targetCategory, this.userId];
            } else {
                query_text = `SELECT content_structured
                   FROM domain_knowledge
                   WHERE domain = 'payments'
                   ...
                   ORDER BY RANDOM()
                   LIMIT 1`;
                query_params = [targetCategory, this.userId];
            }
        } else {
            ...
            if (subtopic) {
                query_text = `SELECT content_structured
                   ...
                   ORDER BY RANDOM()
                   LIMIT 1`;
                query_params = [subtopic, this.userId];
            } else {
                query_text = `SELECT content_structured
                   FROM domain_knowledge
                   WHERE domain = 'payments'
    ...
    getScaleInfo() {
        return {
            ...adaptiveDifficultyScaler.getInstance(this.userId).getScaleInfo(),
...
Oggy correctly categorized this during autonomous practice and added it to domain knowledge for future learning.`;
            ...
            `, [
                ...
                'shareable',
                2,
                JSON.stringify(['categorization', 'self_learned', category, this.userId]),
                content_hash,
                this.userId
            ]);
...
function getInstance(userId) {
    if (!instances.has(userId)) {
        instances.set(userId, new SelfDrivenLearning());
    }
    return instances.get(userId);
}

module.exports = { getInstance, SelfDrivenLearning };
