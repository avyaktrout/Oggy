...
class SelfDrivenLearning {
     ...
     * This ensures Oggy learns NEW patterns while reinforcing existing ones
     */
    async _generateAssessment() {
        ...
        if (useTessaGeneration) {
            ...
            try {
                const scenario = await tessaAssessmentGenerator.generateNovelScenario({
                    difficulty: this._randomDifficulty()
                });
        ...
        return await this._generateFromDomainKnowledge();
    }

    /**
     * Generate assessment from existing domain knowledge
     */
    async _generateFromDomainKnowledge() {
    ...
    /**
     * Adaptive difficulty selection based on Oggy's recent performance
     * - High accuracy (>80%): Challenge with harder scenarios
     * - Medium accuracy (60-80%): Mix of medium and hard
     * - Low accuracy (40-60%): Focus on medium difficulty
     * - Very low (<40%): Use easier scenarios
     */
    _randomDifficulty() {
        // If not enough data yet, use medium difficulty
        if (this.recentAccuracy.length < 5) {
            return 'medium';
        }

        // Calculate recent accuracy
        const sum = this.recentAccuracy.reduce((a, b) => a + b, 0);
        const avgAccuracy = sum / this.recentAccuracy.length;

        logger.debug('Adaptive difficulty calculation', {
            recent_accuracy: avgAccuracy,
            sample_size: this.recentAccuracy.length
        });
