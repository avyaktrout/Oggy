...
-- Ensure constraint includes all question types (CREATE TABLE IF NOT EXISTS won't update existing constraints)
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        AND pg_get_constraintdef(oid) LIKE '%ai_advice%'
    ) THEN
...
CREATE INDEX IF NOT EXISTS idx_inquiries_user_topic
    ON oggy_inquiries (user_id, topic) WHERE status IN ('pending', 'answered');

-- Add topic column to existing tables
ALTER TABLE oggy_inquiries ADD COLUMN IF NOT EXISTS topic TEXT;
